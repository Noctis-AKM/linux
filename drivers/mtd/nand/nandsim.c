...
#define NSMTD_SIZE	(sizeof(struct nand_chip) + sizeof(struct nandsim))
...
static int __init ns_init_module(void)
{
	...
	for (mtd_id = 0; mtd_id < dev_count; mtd_id++) {
		chip        = (struct nand_chip *)(((void *)nsmtds) + NSMTD_SIZE * mtd_id);
...
error:
	while (--mtd_id >= 0) {
        chip = (struct nand_chip *)(((void *)nsmtds) + NSMTD_SIZE * mtd_id);
		nsmtd = nand_to_mtd(chip);
		nand = nand_get_controller_data(chip);
		free_nandsim(nand);    /* Free nandsim private resources */
